...
import { AttributesInitialConfig, FormFieldsInitialConfig, InitialConfig } from "./InitialConfig";
...
export type FormFieldsQueryBuilderConfigWithRaqbFields = ReturnType<typeof getQueryBuilderConfig>;
...
export function getQueryBuilderConfig(form: RoutingForm, forReporting = false) {
  ...
  form.fields?.forEach((field) => {
    ...
    if (FieldTypes.map((f) => f.value).includes(field.type as RoutingFormFieldType)) {
  ...
  const initialConfigCopy = {
    ...InitialConfig,
    operators: { ...InitialConfig.operators } as typeof InitialConfig.operators & {
