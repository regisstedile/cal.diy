import { canEditEntity, entityPrismaWhereClause } from "@calcom/lib/entityPermissionUtils";
...
export const formMutationHandler = async ({ ctx, input }: FormMutationHandlerOptions) => {
  ...
  return await prisma.app_RoutingForms_Form.upsert({
    ...
    create: {
      ...
      description,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      routes: (routes === null ? null : routes) as any,
      id: id,
      ...(teamId
        ? {
            team: {
    ...
    update: {
      ...
      description,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      settings: (settings === null ? null : settings) as any,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      routes: (routes === null ? null : routes) as any,
    },
  });
